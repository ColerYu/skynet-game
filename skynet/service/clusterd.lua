...
local function loadconfig()
	...
	for name, nodeinfo in pairs(tmp) do
		assert(type(nodeinfo) == "table")
		assert(type(nodeinfo.nodeaddr) == "string")
		if node_address[name] ~= nodeinfo.nodeaddr then
			-- address changed
			if rawget(node_channel, name) then
				node_channel[name] = nil	-- reset connection
			end
			node_address[name] = nodeinfo.nodeaddr
		end
	end
	--[[
