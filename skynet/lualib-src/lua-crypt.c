...
//异或方式加解密,可对二进制数据进行操作
static int
lxor_data(lua_State *L){
	int i;
	size_t sz, len;
	if (lua_isstring(L, 1) == 1) //传入的是string
	{	
		const char * pbuf = luaL_checklstring(L, 1, &sz);
		len = luaL_checkinteger(L, 2);
		char * buffer = lua_newuserdata(L, len);
		char * result = buffer;
		for (i=0; i<len; i++)
		{
			char c = *pbuf;
			if(c & 0x01)
				*buffer = (c & 0x07) | (0xf8 & ~(c & 0xf8));
			else if(c & 0x04)
				*buffer = (c &0xc7) | (0x38 & ~(c & 0x38));
			else
				*buffer = (c & 0x47) | (0xb8 & ~(c & 0xb8));
			buffer++;
			pbuf++;
		}
		lua_pushlstring(L, result, len);
	}
	else if (lua_islightuserdata(L, 1) == 1) //传入的是指针直接不用返回
	{
		char * pbuf = (char *)lua_touserdata(L,1);
		...
		for (i=0; i<len; i++)
		{
			char c = *pbuf;
			if(c & 0x01)
				*pbuf = (c & 0x07) | (0xf8 & ~(c & 0xf8));
			else if(c & 0x04)
				*pbuf = (c &0xc7) | (0x38 & ~(c & 0x38));
			else
				*pbuf = (c & 0x47) | (0xb8 & ~(c & 0xb8));
			pbuf++;
		}
		lua_pushlstring(L, result, len);
	...
	{
		return luaL_error(L, "args 1 not string or ptr");
	}
